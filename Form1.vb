...
#End Region
    ...
    Dim title As String = "ESD在线状态看板 [ESD on line protection system for electrostatic alarm] "
    ''' <summary>
    ''' 程序启动,初始化
    ''' </summary>
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.Text = title & getver

        NotifyIcon1.Text = "ESD在线状态看板"
        '关联ESD点击事件
        ...
        Panel3.Location = New Point(Panel1.Width - Panel3.Width, (Panel1.Height / 2) - (Panel3.Height / 2)) '颜色标识

        PictureBox2.Location = New Point((Panel1.Width / 2) - (Label1.Width / 2) - 160, PictureBox2.Location.Y)
        ...
        '没有找到更新的事件
        AddHandler _Update.Updater.NoUpdatesFound, New EventHandler(AddressOf updater_NoUpdatesFound)

        '当检查发生错误时,这个事件会触发
        AddHandler _Update.Updater.Error, New EventHandler(AddressOf updater_Error)

        '找到更新的事件.但在此实例中, 找到更新会自动进行处理, 所以这里并不需要操作
        AddHandler _Update.Updater.UpdatesFound, New EventHandler(AddressOf updater_UpdatesFound)
        ...
        ''''串口监控
        'If SC_Relay_control_Left.IsOpen Then
        '    DataRecerice()
        'End If
        ...
        Dim thread As Thread = New Threading.Thread(AddressOf OpenOrClosePort)
...
#Region "更新"
    Dim _Update As New UpdateClass
    ''' <summary>
    ''' 没有找到更新的事件
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub updater_NoUpdatesFound(ByVal sender As System.Object, ByVal e As System.EventArgs)
        '最新版本就启动按钮
        Me.Text = title & " v" & Application.ProductVersion.ToString & "  (已是最新版本" & _Update.Get_URL & ")"
    End Sub

    Dim _UpBol As Boolean = False '防止更新错误后重复启动更新
    ''' <summary>
    ''' 更新错误
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub updater_Error(ByVal sender As System.Object, ByVal e As System.EventArgs)
        '应该是版本问题,官方给的获取升级错误的例子不能正确使用
        Dim updater = TryCast(sender, FSLib.App.SimpleUpdater.Updater)
        'MessageBox.Show(updater.e.ToString)
        Me.Text = title & " v" & Application.ProductVersion.ToString & "  (更新错误" & _Update.Get_URL & ")"

        If _UpBol = False Then
            _UpBol = True
            '更新错误就更改URL再尝试一次更新
            _Update.StartUpdate("http://10.114.142.49/update/ESDKANBAN/update.xml")
        End If
    End Sub
    ''' <summary>
    ''' 找到更新事件
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub updater_UpdatesFound(ByVal sender As System.Object, ByVal e As System.EventArgs)
        '找到更新
        Me.Text = title & " v" & Application.ProductVersion.ToString & "  (找到新的版本" & _Update.Get_URL & ")"
    End Sub
#End Region
    ''' <summary>
    ''' 数据库更新操作线程
...
#Region "采集器A 串口接收事件"
    ...
    Dim LeftUpdataTime As DateTime

    Dim leftrecordsum As Integer = 0
    ''' <summary>
...
#Region "线体左边采集器串口状态采集"
    ...
    Private Sub thread_senddate_Left()
        Log("A线程循环发送串口指令 启动")
        Do
            If thread_bol = False Or S1 = False Then
                '延迟1秒
...
#Region "线体右边采集器串口状态采集"
    ...
    Private Sub thread_senddate_right()
        ...
        Do
            If thread_bol = False Or S3 = False Then
                '延迟1秒
                Threading.Thread.Sleep(1000)
                Continue Do
            End If
    ...
    Private Sub Statistical_chart()
        ...
        Do
            Try
                ...
                Dim sql As String = "select top 30 t_ESD_number as ESD编号 ,count(t_ESD_number) as 次数,sum(t_Duration_time) as 持续时间, (sum(t_Duration_time) / count(t_Duration_time)) as 平均持续时长 from t_ESD_status_record where t_Creation_time between '" & data1 &
                    " 08:00:00' and '" & data2 & " 08:00:00' and t_Line_body = '" & t_Line_body & "' and t_ESD_state like '%失败%' group by t_ESD_number order by count(t_Duration_time) desc"
                ...
                For Each T In Getlist
                    If T.Count = 4 Then
                        Dim tlist As New Set_indexes With {
                                          ...
                                          .Get_Abnormal_time = IIf(T(2).ToString <> "", T(2).ToString, 0),
                                          .Get_average_time = IIf(T(3).ToString <> "", T(3).ToString, 0)}
                        getcount += 1
...
#End Region
    ...
    Private Sub ToolStripButton2_Click(sender As Object, e As EventArgs) Handles ToolStripButton2.Click
        System.Diagnostics.Process.Start("http://10.124.149.29:8020/ESD_HELP/ESD_help.html")
    End Sub




    Dim recordsum As Integer
    ''' <summary>
    ''' 数据监测（缓冲区大小）
    ''' </summary>
    Private Sub DataRecerice()

        Dim BytesToRead As Integer
        BytesToRead = SC_Relay_control_Left.BytesToRead()
        ...
        Try
            ...
            If (recordsum Mod 2) = 0 Then

                'Button9_Click(Nothing, Nothing)
                'Button5_Click(Nothing, Nothing)

                SC_Relay_control_Left.closePort()
                SC_Relay_control_right.closePort()
                ...
                Thread.Sleep(2000)
                SC_Relay_control_Left.openPort()
                SC_Relay_control_right.openPort()
                Button9.BackColor = Color.DarkSeaGreen '绿色
    ...
    ''' 数据监测（响应时间）
    ''' 刘冲创建
    ''' 2019-10-11 索圆修改[当超过5分钟超时时间后将时间同步为当前时间]
    ''' </summary>
    Private Sub OpenOrClosePort()
        ...
        SyncLock lockObj
            While True
                ...
                Try
                    ...
                    If DateDiff(DateInterval.Second, LeftUpdataTime, DateTimeNow) > 5 Then
                        Me.Invoke(New Action(Sub()
                                                 'thread_bol = False
                                                 'Button5_Click(Nothing, Nothing)
                                                 LeftUpdataTime = Now
                                             ...
                                             End Sub))
                    End If
                    If DateDiff(DateInterval.Second, RightUpTime, DateTimeNow) > 5 Then
                        Me.Invoke(New Action(Sub()
                                                 'thread_bol = False
                                                 'Button9_Click(Nothing, Nothing)
                                                 RightUpTime = Now
